...
export async function POST(req: Request) {
  try {
    ...
    const result = await exa.searchAndContents(searchTerm.searchTerm, {
      ...
      summary: {
        query:
          "Summarize the content of the text, try to incude the main ideas backed with justification from the abstract, author or paper source.",
      },
    });
    ...
    const createdPapers = await prisma.$transaction(async (tx) => {
      ...
      // Create new papers and get their IDs
      const papers = await Promise.all(
        result.results.map((paper) =>
          tx.savedPaper.create({
            data: {
              paperId: paper.id,
    ...
    return NextResponse.json({ allPapers: createdPapers });
